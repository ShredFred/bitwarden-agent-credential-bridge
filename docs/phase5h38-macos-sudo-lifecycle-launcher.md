...
The launch has a 130-second outer child deadline, covering the 120-second consent window and a bounded execution margin. Once the challenge has been answered, the launcher records its runner PID. Any later failure kills that PID only after a fresh `libproc` snapshot proves the same start time, root UID, and exact fixed runner path; it then also terminates and reaps the isolated sudo process group. This avoids both an orphaned privileged runner and a PID-reuse kill of an unrelated process.
