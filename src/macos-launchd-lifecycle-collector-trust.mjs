...
/**
 * Phase 11a: pure macOS elevated-collector provenance evaluator.
 *
 * Synthetic provenance may satisfy the schema in tests; live verification stays
 * false until a Mac-host disposable collector exists (Phase 11c).
 */
...
const PROVENANCE_FIELDS = new Set([
  ...
  'sudo_consent_observed',
  'root_euid_reported',
  'system_domain_reported',
]);
...
export class MacosLaunchdLifecycleCollectorTrustError extends Error {
  constructor(code = 'invalid_provenance') {
    super(`macOS launchd lifecycle collector trust rejected: ${code}`);
    this.name = 'MacosLaunchdLifecycleCollectorTrustError';
    this.code = code;
  }
}
...
export function buildMacosLaunchdLifecycleCollectorContract(gate) {
  if (!isMacosLaunchdLifecycleGate(gate)) {
    throw new MacosLaunchdLifecycleCollectorTrustError('invalid_gate');
  }
  const contract = deepFreeze({
    ...
    requirements: {
      ...
      same_euid_helper_forbidden: true,
    },
...
export function isMacosLaunchdLifecycleCollectorContract(value) {
  return value !== null && typeof value === 'object' && VALID_CONTRACTS.has(value);
}

export function evaluateMacosLaunchdLifecycleCollectorTrust(gate, transcript, provenance) {
  if (!isMacosLaunchdLifecycleGate(gate)) {
    throw new MacosLaunchdLifecycleCollectorTrustError('invalid_gate');
  }
  const structure = evaluateMacosLaunchdLifecycleTranscript(gate, transcript);
  ...
  if (facts.schema_version !== 1 ||
    ...
    throw new MacosLaunchdLifecycleCollectorTrustError();
  }
