...
- Linux SM token store opens with `O_NOFOLLOW`, requires current-UID ownership
  and owner-only parent/file modes, refuses relative and Windows-style paths
  plus symlink parents, inspects the parent with `lstat` before `chmod`, and
  cleans leftover temp files. GUI zenity/kdialog runs with a bounded capture
  that stops appending and SIGKILLs after overflow, a minimal environment
  (no `BWS_ACCESS_TOKEN`), and only `/usr/bin` binaries that are not
