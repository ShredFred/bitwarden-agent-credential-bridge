...
# SM machine token path requires the SM flag and forbids personal PM vault flag.
if (-not [bool]$obj.SecretsManagerAllowed) {
    [Console]::Error.Write('secrets_manager_required')
    exit 14
}
...
# Emit only the access token bytes to stdout. Callers must never log stdout.
